...
CREATE TABLE REIMBURSEMENT(
    ...
    REIMBURSEMENT_PROJECTED       NUMBER(*,2)
);
...
/*STORED PROCEDURES*/
CREATE OR REPLACE PROCEDURE APPLY_RIM
(EMPID IN NUMBER,TFEES IN NUMBER,ETYPE IN NUMBER,EDATE IN DATE,EPLACE IN VARCHAR2,
EDESC IN VARCHAR2,GFORMAT IN NUMBER,EJUSTIFY IN VARCHAR2,MDAYS IN NUMBER,RPROJECT IN NUMBER)
AS
EID INTEGER;
BAL NUMBER;
...
SELECT EMPLOYEE_BALANCE INTO BAL FROM EMPLOYEE WHERE EMPLOYEE_ID=EMPID;
...
INSERT INTO REIMBURSEMENT (REIMBURSEMENT_EMPLOYEE_ID,REIMBURSEMENT_COST,REIMBURSEMENT_STATUS,REIMBURSEMENT_EVENT,REIMBURSEMENT_DAYS_MISSED,REIMBURSEMENT_JUSTIFICATION,REIMBURSEMENT_PROJECTED) 
VALUES (EMPID,TFEES,'PENDING',EID,MDAYS,EJUSTIFY,RPROJECT);

BAL:=(BAL-RPROJECT);

UPDATE EMPLOYEE 
SET EMPLOYEE_BALANCE=BAL 
...
(RIMID IN NUMBER,EGRADE IN VARCHAR2)
AS
EID INTEGER;

BEGIN

SELECT REIMBURSEMENT_EVENT INTO EID FROM REIMBURSEMENT WHERE REIMBURSEMENT_ID=RIMID;

UPDATE EVENT 
SET EVENT_GRADE=EGRADE 
WHERE EVENT_ID=EID;
...
(RIMID IN NUMBER,STATUS IN VARCHAR2,STATUSBY IN VARCHAR2,NOTES IN VARCHAR2)
AS
EMPID NUMBER;
BAL NUMBER;
RPROJECT NUMBER;
BEGIN
SELECT REIMBURSEMENT_PROJECTED INTO RPROJECT FROM REIMBURSEMENT WHERE REIMBURSEMENT_ID=RIMID;
SELECT REIMBURSEMENT_EMPLOYEE_ID INTO EMPID FROM REIMBURSEMENT WHERE REIMBURSEMENT_ID=RIMID;

SELECT EMPLOYEE_BALANCE INTO BAL FROM EMPLOYEE WHERE EMPLOYEE_ID=EMPID;

UPDATE REIMBURSEMENT SET REIMBURSEMENT_STATUS=STATUS 
WHERE REIMBURSEMENT_ID=RIMID;

UPDATE REIMBURSEMENT SET REIMBURSEMENT_STATUS_BY=STATUSBY 
WHERE REIMBURSEMENT_ID=RIMID;

UPDATE REIMBURSEMENT SET REIMBURSEMENT_NOTES=NOTES 
WHERE REIMBURSEMENT_ID=RIMID;
